...
function procat_ci_ssh_destroy_agent {
	if [ -n "${PROCAT_CI_SSH_AUTH_SOCK-}" ]; then
		pc_log "procat_ci_ssh_destroy_agent()    : Stopping the CI ssh agent...."
		...
		pc_log "procat_ci_ssh_destroy_agent()    : A CI ssh agent is not configured."
	fi
}
